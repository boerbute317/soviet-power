...
INPUT_TYPE = Union[torch.Tensor, List[Union[int, list, torch.Tensor, np.ndarray]], Tuple[
    Union[int, list, torch.Tensor, np.ndarray]], np.ndarray]
...
def repetition_penalty_func(input_ids: torch.LongTensor, scores: torch.FloatTensor,
                            penalty: float) -> torch.FloatTensor:
...
class AutoRegressiveDecoder(object):
    ...
    def __init__(self, bos_token_id: int = None, eos_token_id: int = -1, max_new_tokens: int = None,
                 min_new_tokens: int = 1, max_length: int = 64,
                 pad_token_id: int = 0, pad_mode: Literal['pre', 'left', 'post', 'right'] = 'post', device: str = 'cpu',
                 n: int = 1,
                 top_k: int = None, top_p: float = None, temperature: float = 1, repetition_penalty: float = 1.0,
                 min_ends: int = 1, **generation_config):
        # generation_config
    ...
    def wraps(default_rtype: Literal['probas', 'logits'] = 'probas', use_states: bool = False):
        ...
        def actual_decorator(predict):
            def new_predict(self, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor, states: Optional[dict],
                            rtype=default_rtype):
                ...
                if self.repetition_penalty != 1.0:
                    ...
                    prediction = (
                        repetition_penalty_func(past_token_ids, prediction[0], self.repetition_penalty), prediction[1])
    ...
    def _trans2tensors(self, inputs_raw: INPUT_TYPE) -> list:
        ...
        for input_ in inputs_raw:
            ...
            if isinstance(input_, torch.Tensor):
                ...
                input_new = torch.tensor(sequence_padding(input_, value=self.pad_token_id, mode=self.pad_mode),
                ...
                raise TypeError(
                    f'Beam search inputs ele only support tensor、array、list、tuple, current type is:{type(input_)}'
                )
    ...
    def __beam_search_step(self, step: int, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor,
                           output_scores: torch.Tensor, states: Optional[dict] = None):
        '''beam_search单条推理计算得分'''
        self.step = step
    ...
    def __beam_search_end(self, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor, output_scores: torch.Tensor,
                          results: list):
    ...
    def __batch_beam_search_step(self, step: int, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor,
                                 output_scores: torch.Tensor, states: Optional[dict] = None):
    ...
    def __batch_beam_search_end(self, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor,
                                output_scores: torch.Tensor, results: list):
        break_tag = False
        is_end, end_counts = self._identify_sentence_end(output_ids)
    ...
    def beam_search(self, inputs: Union[torch.Tensor, List[Union[int, list, tuple, torch.Tensor, np.ndarray]], Tuple[
        Union[int, list, tuple, torch.Tensor, np.ndarray]], np.ndarray],
        ...
        for step in self._define_stopping_criteria(states):
            if (not self.use_batch):  # 单条推理
                inputs, output_ids, output_scores, states = self.__beam_search_step(
                    step, inputs,
                    output_ids,
                    output_scores,
                    states
                )
                inputs, output_ids, output_scores, results, break_tag = self.__beam_search_end(
                    inputs,
                    output_ids,
                    output_scores,
                    results
                ...
                inputs, output_ids, output_scores, states = self.__batch_beam_search_step(
                    step, inputs,
                    output_ids,
                    output_scores,
                    states
                )
                inputs, output_ids, output_scores, results, break_tag = self.__batch_beam_search_end(
                    inputs,
                    output_ids,
                    output_scores,
                    results
    ...
    def stream_beam_search(self, inputs: INPUT_TYPE, states: Optional[dict] = None, **generation_config):
        ...
        results = []
        for step in self._define_stopping_criteria(states):
            inputs, output_ids, output_scores, states = self.__beam_search_step(step, inputs, output_ids, output_scores,
                                                                                states)
            ...
            inputs, output_ids, output_scores, results, break_tag = self.__beam_search_end(inputs, output_ids,
    ...
    def __random_sample_step(self, step: int, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor,
    ...
    def __random_sample_end(self, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor, results: list,
                            smp_indexs: torch.Tensor = None):
        break_tag = False
        is_end, end_counts = self._identify_sentence_end(output_ids)
    ...
    def random_sample(self, inputs_raw: INPUT_TYPE, states: Optional[dict] = None, **generation_config):
        ...
        for step in self._define_stopping_criteria(states):
            if not self.use_batch:  # single
                ...
                inputs, output_ids, states = self.__random_sample_step(step, inputs, output_ids, states)
                inputs, output_ids, results, break_tag, smp_indexs = self.__random_sample_end(inputs, output_ids,
                                                                                              results, smp_indexs)
            if break_tag:
                break
...
class SeqGeneration(AutoRegressiveDecoder):
    ...
    def __init__(self, model: BaseModel, tokenizer=None, tokenizer_config: dict = None,
                 mode: Literal['beam_search', 'random_sample'] = 'random_sample',
                 default_rtype: Literal['logits', 'probas'] = 'logits', use_states: bool = True,
                 optimize_cuda_cache: bool = False, **kwargs):
        # 如果传进来的是Trainer
        ...
        self.tokenizer_encode_config = kwargs.get('tokenizer_encode_config') or self.clear_tokenizer_config(
            tokenizer_config, 'encode')
        self.tokenizer_decode_config = kwargs.get('tokenizer_decode_config') or self.clear_tokenizer_config(
        ...
        self.use_segment_ids = hasattr(model, 'segment_vocab_size') and (
                model.segment_vocab_size > 0)  # 是否使用segment_ids
    ...
    def predict(self, inputs: TUPLE_LIST_TENSOR_TYPE, output_ids: torch.Tensor, states: Optional[dict]):
        ...
        if self.use_states:
            ...
            if self.step == 0:
                ...
                if self.use_batch and (self.pad_mode in {'pre', 'left'}):
                    ...
                    #         [0, 0, 0, 0, 0, 1, 2, 3, 4, 5]])
                    states['position_ids'] = create_position_ids_start_at_padding(next_inputs[0], self.pad_token_id,
                                                                                  past_key_values_length=-1,
                                                                                  start_padding_idx=False)
                ...
                if states.get('pad_attention_mask') is not None:  # 在states中input_attention_mask才是[btz, seq_len]
                    ...
                    states['attention_mask'] = torch.cat(
                ...
                if (self.mode == 'beam_search' or self.n > 1) and (self.step == 1):
                    ...
                    if (states.get('past_key_values') is not None) and states['past_key_values'][0][0].shape[0] != btz:
                        ...
                        for l_i, past_key_value in enumerate(states['past_key_values']):
                            states['past_key_values'][l_i] = (past_key_value[0].repeat_interleave(repeat_size, dim=0),
                                                              past_key_value[1].repeat_interleave(repeat_size, dim=0))
                    if (states.get('cross_past_key_values') is not None) and \
                            states['cross_past_key_values'][0][0].shape[0] != btz:
                        ...
                        for l_i, cross_past_key_value in enumerate(states['cross_past_key_values']):
                            states['cross_past_key_values'][l_i] = (
                                cross_past_key_value[0].repeat_interleave(repeat_size, dim=0),
                                cross_past_key_value[1].repeat_interleave(repeat_size, dim=0))
                ...
                if hasattr(self, 'flag') and (self.flag is not None) and (self.flag == 0).sum().item() > 0:
                    ...
                    states['past_key_values'] = [(kv[0][self.flag], kv[1][self.flag]) for kv in
                                                 states['past_key_values']]
                    if 'cross_past_key_values' in states:
                        states['cross_past_key_values'] = [(kv[0][self.flag], kv[1][self.flag]) for kv in
                                                           states['cross_past_key_values']]
        ...
        # 不使用cache
            ...
            if self.pad_mode in {'pre', 'left'}:
                position_ids = create_position_ids_start_at_padding(next_inputs[0], self.pad_token_id,
                                                                    past_key_values_length=-1, start_padding_idx=False)
    ...
    def clear_tokenizer_config(self, config: dict, mode: str):
        ...
        encode_kwargs = {'max_length', 'add_special_tokens', 'padding', 'truncation', 'stride', 'allowed_special',
                         'return_tensors',
    ...
    def pre_process(self,
                    text: Union[str, torch.Tensor, List[Union[int, torch.Tensor]], Tuple[Union[int, torch.Tensor]]]) -> \
    ...
    def post_process(self, outputs):
        ...
        if len(output_ids) > 1:
            ...
            output_text = self.input_text + self.tokenizer.decode(output_ids[0].cpu().numpy(),
                                                                  **self.tokenizer_decode_config)
    ...
    def generate(self, text: Union[str, list, torch.Tensor], **kwargs):
        ...
        if isinstance(text, (str, torch.Tensor)):
            ...
            if self.use_states and (self.pad_mode in {'post', 'right'}):
                self.pad_mode = 'pre'
                log_info(
                    "When arg `use_states`=True, you may set `pad_mode`='pre' to avoid error output, reset `pad_mode`='pre' instead")
        else:
...
class Seq2SeqGeneration(SeqGeneration):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.use_segment_ids = hasattr(self.encoder, 'segment_vocab_size') and (
                self.encoder.segment_vocab_size > 0)  # 是否使用segment_ids
