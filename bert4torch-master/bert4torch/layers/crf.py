...
class CRF(nn.Module):
    ...
    def __init__(self, num_tags: int, init_transitions: Optional[List[np.ndarray]] = None, freeze=False) -> None:
        ...
        if (init_transitions is None) and (not freeze):
            self.start_transitions = nn.Parameter(torch.empty(num_tags))  # 初始状态概率矩阵
            self.end_transitions = nn.Parameter(torch.empty(num_tags))  # 结尾的状态概率矩阵
            self.transitions = nn.Parameter(torch.empty(num_tags, num_tags))  # 状态与状态之间转换可能性的概率矩阵
    ...
    def _viterbi_decode_nbest(self, emissions: torch.FloatTensor, mask: torch.ByteTensor,
        ...
        # shape: (batch_size, num_tags)
        score = self.start_transitions + emissions[:, 0]  # 第一个token属于各个类别的置信度大小 [batch_size, num_tags]
        ...
        for i in range(1, seq_length):
            if i == 1:
                # 上一个token属于各个类别的置信度 [batch_size, num_tags] -> [batch_size, num_tags,1]
                broadcast_score = score.unsqueeze(-1)
                # 当前token到观察值(特征属性)之间的置信度(FC输出的输出各个类别的置信度值) [batch_size, 1, num_tags]
                broadcast_emission = emissions[:, i].unsqueeze(1)
                # shape: (batch_size, num_tags, num_tags)
                # [b,n,1] + [n,n] + [b,1,n] --> 针对每个样本，得到从上一个token输出各个类别的置信度到当前token属于各个类别置信度的和
        ...
        for idx in range(seq_length - 1, -1, -1):
            ...
            best_tags_arr[:, idx] = torch_div(best_tags.data.view(batch_size, -1), nbest,
                                              rounding_mode='floor')  # 兼容老版本
