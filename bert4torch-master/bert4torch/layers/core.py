...
class LayerNorm(nn.Module):
    def __init__(self, hidden_size: int, eps: float = 1e-12, conditional_size: Union[bool, int] = False,
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_size: int, hidden_size: int, max_position: int,
                 segment_vocab_size: int, shared_segment_embeddings: bool,
        ...
        # LayerNorm
        self.layerNorm = LayerNorm(embedding_size, eps=kwargs.get('layer_norm_eps', 1e-12),
                                   conditional_size=conditional_size, **kwargs)
    ...
    def apply_hierarchical_pos_embedding(self, position_index):
        ...
        embeddings_x = take_along_dim(embeddings,
                                      torch_div(position_index_reshape, embeddings.size(0), rounding_mode='trunc'),
                                      dim=0)  # 兼容老版本
    ...
    def apply_embeddings(self, token_ids: torch.Tensor, segment_ids: torch.Tensor, position_ids: torch.Tensor,
        ...
        if hasattr(self, 'position_embeddings') and (position_ids is not None):
            ...
            if (self.hierarchical_position is not None) and (
                    position_ids.shape[1] > self.position_embeddings.weight.shape[0]):
                # 层次分解位置编码
    ...
    def forward(self, token_ids: torch.Tensor = None, segment_ids: torch.Tensor = None,
                position_ids: torch.Tensor = None, conditional_emb: Optional[torch.Tensor] = None,
                additional_embs: Union[Tuple[torch.Tensor], List[torch.Tensor]] = None,
...
class PositionWiseFeedForward(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int, dropout_rate: float = 0.5, hidden_act: str = 'gelu',
                 is_dropout: bool = False, bias: bool = True, **kwargs):
        ...
        self.intermediateDense = nn.Linear(hidden_size,
                                           intermediate_size * 2 if hidden_act == 'swiglu' else intermediate_size,
                                           bias=bias)
...
class LlamaFeedForward(nn.Module):
    ...
    def forward(self, x):
        z1 = self.intermediate_act_fn(self.intermediateDense(x))  # 相当于门结构
        z2 = self.intermediateDense2(x)  # 普通全连接获取特征属性
        x = z1 * z2  # 激活操作
