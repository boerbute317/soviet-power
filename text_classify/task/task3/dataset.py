...
class ListDataset(Dataset):
    def __init__(self, X, Y, token_id_2_stroke_n_grams):
        super(ListDataset, self).__init__()
        self.X = X  # list(list(int)) 里面的存储的是每个文本对应的token id列表
        self.Y = Y  # list(int) 里面存储的是每个文本对应的类别id
        self.token_id_2_stroke_n_grams = token_id_2_stroke_n_grams
    ...
    def __getitem__(self, i):
        ...
        _y = self.Y[i]  # 数字  --> int
        _x_stroke_list = []  # 每个token对应的笔画n-gram的list --> list(list(int))
        __x_stroke_len = []
        for _t_id in _x:
            _stroke_id = copy.deepcopy(
                self.token_id_2_stroke_n_grams[_t_id]
            )
            _x_stroke_list.append(_stroke_id)
            __x_stroke_len.append(len(_stroke_id))
        return copy.deepcopy(_x), _y, len(_x), _x_stroke_list, __x_stroke_len

    @staticmethod
    def collate_fn(batch):
        batch_x, batch_y, batch_x_length, batch_stroke, batch_stroke_length = list(zip(*batch))
        max_token_length = max(batch_x_length)  # 也就是T
        max_stroke_length = max([max(t) for t in batch_stroke_length])  # 也就是L
        mask = []
        stroke_mask = []
        for i in range(len(batch_x)):
            _x = batch_x[i]
            _mask = np.zeros(max_token_length)
            _mask[:len(_x)] = 1
            mask.append(list(_mask))
            if len(_x) < max_token_length:
                _x.extend([0] * (max_token_length - len(_x)))

            _stroke_mask = np.zeros((max_token_length, max_stroke_length))
            for j in range(len(batch_stroke[i])):
                _stroke_mask[j][:batch_stroke_length[i][j]] = 1
                if batch_stroke_length[i][j] < max_stroke_length:
                    batch_stroke[i][j].extend([0] * (max_stroke_length - batch_stroke_length[i][j]))
            if len(batch_stroke[i]) < max_token_length:
                zero_mask = [0] * max_stroke_length
                batch_stroke[i].extend([zero_mask] * (max_token_length - len(batch_stroke[i])))
            stroke_mask.append(list(_stroke_mask))

        batch_x = torch.tensor(batch_x, dtype=torch.int64)  # [N,T]
        batch_stroke = torch.tensor(batch_stroke, dtype=torch.int64)  # [N,T,L]
        batch_y = torch.tensor(batch_y, dtype=torch.int64)  # [N]
        mask = torch.tensor(mask, dtype=torch.float32)  # [N,T] 实际值位置为1，填充值位置为0
        batch_stroke_mask = torch.tensor(stroke_mask, dtype=torch.float32)  # [N,T, L] 实际值位置为1，填充值位置为0
        return (batch_x, mask, batch_stroke, batch_stroke_mask), batch_y
...
def get_word_strokeids(
    ...
    if chchar not in chchar2stroke:
        return [1]

    strokestr = chchar2stroke[chchar]  # 获取当前字对应的笔画
...
def fetch_dataloader(data_path_dir, batch_size):
    ...
    # 从文件加载token/字对应的笔画list列表
    chchar2stroke = load_strokes(os.path.join(data_path_dir, "strokes.txt"))
    strokengram2id = load_stroke_ids()

    # 构建词典/字典 token和token id的映射mapping
    token_2_idx = {
        '<PAD>': 0, '<UNK>': 1
    }
    token_id_2_stroke_n_grams = {
        '<PAD>': [1],
        '<UNK>': [1],
    }
    X = []
    for text in df.x.values:
        ...
        for token in list(text):
            ...
            x.append(token_id)
            token_id_2_stroke_n_grams[token_id] = get_word_strokeids(
                chchar2stroke,  # token到笔画的映射mapping
                token,
                strokengram2id,  # 各个n-gram组合到id的映射mapping
                smallest_n=3,
                biggest_n=5
            )
        X.append(x)
    ...
    train_dataset = ListDataset(X_train, y_train, token_id_2_stroke_n_grams)
    test_dataset = ListDataset(X_test, y_test, token_id_2_stroke_n_grams)
    ...
    return train_dataloader, test_dataloader, token_2_idx, len(yidx2label), weights, strokengram2id
