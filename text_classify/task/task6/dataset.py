...
class ListDataset(Dataset):
    def __init__(self, X, Y):
        ...
        self.X = X  # list(str) 里面的存储的是每个文本对应的text列表
    ...
    def build_collate_fn(tokenizer):
        def collate_fn(batch):
            ...
            input_tokens = tokenizer(batch_x, padding=True)
            batch_x = torch.tensor(input_tokens['input_ids'], dtype=torch.int64)  # [N,T]
            mask = torch.tensor(input_tokens['attention_mask'], dtype=torch.float32)  # [N,T] 实际值位置为1，填充值位置为0
...
def fetch_dataloader(data_path_dir, batch_size, bert_model_id):
    data_path = os.path.join(data_path_dir, "train.csv")
    ...
    df['y'] = df.y.apply(lambda label: ylabel2idx[label])

    X_train, X_test, y_train, y_test = train_test_split(df.x.values, list(df.y.values), test_size=0.2, random_state=28)
    ...
    train_dataloader = DataLoader(
        ...
        collate_fn=ListDataset.build_collate_fn(bert_tokenizer),  # 是一个方法，功能是定义了如何将dataset返回的对象转换/封装为批次对象
    ...
    test_dataloader = DataLoader(
        ...
        collate_fn=ListDataset.build_collate_fn(bert_tokenizer)
    )

    return train_dataloader, test_dataloader, len(yidx2label), weights, bert_tokenizer
