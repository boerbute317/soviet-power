...
from text_classify.task.task6.dataset import fetch_dataloader
from text_classify.task.task6.network import NetworkBertV1
...
def training():
    bert_model_id = r"D:\huggingface\huggingface\hub\models--bert-base-chinese"
    # 初始化
    now_str = datetime.now().strftime("%Y%m%d%H%M%S")
    # now_str = "20240607202638"
    # 分字数据的输入
    train_dataloader, test_dataloader, num_classes, class_weight, bert_tokenizer = fetch_dataloader(
...
def export_static_model():
    ...
    :return:
    """
    root_dir = f'{FILE_ROOT_DIR}/output/20240617203822'
    bert_model_id = r"D:\huggingface\huggingface\hub\models--bert-base-chinese"
    ...
    if example_x.shape[0] == 2 and example_x.shape[1] == 8:
        onnx_v1_output = onnx_model_v1.run(
            ...
            input_feed={
                ...
                "attention_mask": example_mask.numpy()
            })[0]
    else:
        onnx_v1_output = None
